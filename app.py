...
# --- 首页 ---
def view_home():
    st.markdown("""<div class="hero-container"><div class="hero-title">抖音爆款工场 Pro</div><div class="hero-sub">让流量不再是玄学 · 专为素人 KOC 打造的 AI 变现神器</div></div>""", unsafe_allow_html=True)
    ...
    with c1:
        with st.container(border=True):
            ...
            st.button("立即使用 ➜", key="h_btn1", on_click=lambda: st.session_state.update({'nav_menu': '📝 文案改写', 'sb_radio': '📝 文案改写'}), type="primary", use_container_width=True)
    with c2:
        with st.container(border=True):
            ...
            st.button("立即使用 ➜", key="h_btn2", on_click=lambda: st.session_state.update({'nav_menu': '💡 爆款选题库', 'sb_radio': '💡 爆款选题库'}), type="primary", use_container_width=True)
    with c3:
        with st.container(border=True):
            ...
            st.button("立即使用 ➜", key="h_btn3", on_click=lambda: st.session_state.update({'nav_menu': '🎨 海报生成', 'sb_radio': '🎨 海报生成'}), type="primary", use_container_width=True)
    with c4:
        with st.container(border=True):
            ...
            st.button("立即使用 ➜", key="h_btn4", on_click=lambda: st.session_state.update({'nav_menu': '🏷️ 账号起名', 'sb_radio': '🏷️ 账号起名'}), type="primary", use_container_width=True)
...
def main():
    if 'user_phone' not in st.session_state:
        ...
        with st.sidebar:
            ...
            st.markdown("---")
            if st.button("🚪 退出", type="secondary"): del st.session_state['user_phone']; st.rerun()
