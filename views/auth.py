...
def view_auth():
    # --- 1. 物理级隔离 CSS：彻底消除双层边框与重影 ---
    st.markdown("""
        <style>
            /* 1. 彻底抹除 Streamlit 指令提示 */
            [data-testid="stFormInstructions"] { display: none !important; }

            /* 2. 核心：强制去掉所有内层边框和阴影，只在最外层容器保留一个边框 */
            /* 锁定包含 input 和小眼睛的唯一物理外壳 */
            [data-testid="stTextInput"] div[data-baseweb="input"],
            [data-testid="stPasswordInput"] div[data-baseweb="input"] {
                background-color: white !important; /* 纯白底色 */
                border: 1px solid #E2E8F0 !important; /* 这是唯一的边框 */
                border-radius: 6px !important;
                box-shadow: none !important; /* 彻底去掉重影阴影 */
            }

            /* 3. 强制内部所有子元素边框归零（解决双层叠加的关键） */
            [data-testid="stTextInput"] input, 
            [data-testid="stPasswordInput"] input,
            [data-testid="stPasswordInput"] button,
            [data-baseweb="input"] > div {
                border: none !important; /* 掐断内层边框 */
                background-color: transparent !important; /* 确保不产生色差 */
                box-shadow: none !important; /* 掐断聚焦时的蓝光阴影 */
    ...
    with card_container:
        with st.container(border=True):
            ...
            with col_r:
                ...
                with t1:
                    with st.form("f_login_pro_v12", border=False):
                ...
                with t2:
                    with st.form("f_reg_pro_v12", border=False):
