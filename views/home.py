...
def view_home():
    # 1. 悬浮岛头图 V4
    st.markdown("""
    <div class="header-card-v4">
        <div class="header-title-v4">抖音爆款工场 Pro</div>
        <div class="header-sub-v4">全流程 AI 创作工作台 · 赋能内容生产 · 连接商业变现</div>
    </div>
    ...
    # === A. 核心功能区 V4 ===
    st.markdown('<div class="section-title-v4">🚀 核心创作引擎</div>', unsafe_allow_html=True)
    ...
    for i, (icon, title, desc, target) in enumerate(features):
        with [c1, c2, c3, c4][i]:
            st.markdown(f"""
            <div class="feature-box-v4">
                <div class="feat-icon-v4">{icon}</div>
                <div class="feat-title-v4">{title}</div>
                <div class="feat-desc-v4">{desc}</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    <div class="news-box-v4">
        <div class="news-tag-v4">🔔 NEW</div>
        <div class="news-text-v4">{latest_ann}</div>
    </div>
    """, unsafe_allow_html=True)

    # === C. 热门变现任务 (V4: 一体化卡片组件) ===
    st.markdown("<div style='height:20px'></div>", unsafe_allow_html=True)
    st.markdown('<div class="section-title-v4">🔥 热门变现项目</div>', unsafe_allow_html=True)
